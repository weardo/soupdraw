# draw.me — augmented gesture drawing on your live camera
...
- ✅ Transparent `getUserMedia` interception (generic — works on any site that
  requests the camera)
- ✅ On-device **trained gesture model** (MediaPipe Gesture Recognizer, WASM,
  bundled) — returns hand landmarks + a gesture label
- ✅ **Pinch to draw** · **Victory ✌️ to grab & move a shape** · **fist to erase**
  — three unmistakable, model-verified poses (pinch geometry + trained
  `Victory`/`Closed_Fist`, majority-vote smoothed). Erasing splits lines and
  deletes whole shapes it touches.
- ✅ **Shape assist**: a rough circle / line / rectangle / triangle snaps to a
  clean shape — but only when it clearly fits one (confidence-gated, so freehand
  and squiggles are left alone). Works for **multi-stroke** figures too (a box
  drawn as 4 separate strokes, a triangle as 3) and for rounded/under-drawn shapes.
- ✅ **Live gesture HUD** (toggleable) shows what the model detects each frame.
- ✅ Mirror (selfie) view, low-light boost, shape assist, color, size, clear — from the popup
- ✅ Live status readout shows the recognized gesture (tracking / drawing / fps)

Later phases (not built yet): move/drag objects · shapes & stamps · **local
voice ("say a phrase → shape appears")** via bundled Vosk.
...
The draw.me icon appears in the toolbar. (Temporary add-ons unload when Firefox
...
3. Click the draw.me toolbar icon → turn **Augment my camera** ON. Pick a color.
